        ...
        rp(options)
            ...
            .then(function (getArticleUrl) {
                ...
                for (let i = 0; i < arrayOfResultsFromEachPreviousRequest.length; i++) {
                    ...
                    firebase.database().ref('life/').orderByChild("articleId").equalTo(allData[i].articleId).once("value", snapshot => {
    ...
    rp(options)
        ...
        .then(function (arrayOfResultsFromEachPreviousRequest) {
            for (let i = 0; i < arrayOfResultsFromEachPreviousRequest.length; i++) {
                ...
                firebase.database().ref('life/').orderByChild("articleId").equalTo(newTenArticle[i].articleId).once("value", snapshot => {
                    const articleId = snapshot.val()
                    if (articleId) {
                        //console.log("exists!")
                    } else {
