...
router.get('/:time', checkAuth, function (req, res) {
    ...
    timer = schedule.scheduleJob(date, function () {
        const options = {
            uri: 'https://www.gvm.com.tw/category.html?cg_no=2',
        ...
        rp(options)
            ...
            .then(function (getArticleUrl) {
                ...
                for (let i = 0; i < arrayOfResultsFromEachPreviousRequest.length; i++) {
                    ...
                    firebase.database().ref('international/').orderByChild("articleId").equalTo(allData[i].articleId).once("value", snapshot => {
                        const articleId = snapshot.val()
                        if (articleId) {
                            //console.log("exists!")
                        } else {
                            firebase.database().ref('international/').push(
                                allData[i]
                            )
                        }
                    })
    ...
    rp(options)
        ...
        .then(function (arrayOfResultsFromEachPreviousRequest) {
            for (let i = 0; i < arrayOfResultsFromEachPreviousRequest.length; i++) {
                ...
                firebase.database().ref('international/').orderByChild("articleId").equalTo(newTenArticle[i].articleId).once("value", snapshot => {
                    const articleId = snapshot.val()
                    if (articleId) {
                        //console.log("exists!")
                    } else {
